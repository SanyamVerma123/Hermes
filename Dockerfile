# ============================================================
#  Hermes Agent — Self-contained HF Spaces deployment (Stable v5)
#  Single Dockerfile: Gateway + Dashboard + Chat WebUI + Router
#
#  HOW TO DEPLOY:
#  1. Create HF Space → SDK: Docker → app_port: 7860 → Public
...
#
#     HF_TOKEN            → write token from hf.co/settings/tokens
#     HF_BUCKET           → Sanyam400/Hermes-storage (your bucket identifier)
#     
#     API_SERVER_KEY      → random long secret string
#
#     [OPTIONAL TELEGRAM CONFIGURATION]
#     TELEGRAM_BOT_TOKEN  → Your Telegram bot token from @BotFather
#     TELEGRAM_ALLOWED_USERS → Your numeric user ID (comma-separated if multiple)
#
#  4. Wait ~3-4 min build → open your *.hf.space URL in a NEW TAB
#     (not the HF iframe — cookies won't work there)
#
#  URLS:
...
var server = http.createServer(function(req, res) {
    ...
    if (req.method === 'POST') {
      ...
      req.on('end', function() {
        ...
        if (!GATEWAY_TOKEN || tok === GATEWAY_TOKEN) {
          ...
          if (isHttps) {
            cookieFlags += '; Secure; SameSite=None'; // Mandatory for cookies within iFrames
          } else {
  ...
  if (isDashboard) {
    // Keep pathname intact when routing to dashboard to preserve relative sub-router references
...
echo "╔══════════════════════════════════════════╗"
echo "║   Hermes Agent — HF Spaces bootloader    ║"
echo "╚══════════════════════════════════════════╝"
    ...
    for fname in os.listdir(db_dir):
        if fname.endswith(".db"):
            ...
            try:
                ...
                res = cursor.fetchone()
                if res and res == "ok":
                    # Enable Write-Ahead Logging (WAL) and set normal synchronicity
                    # This prevents network sync locks from corrupting SQLite dbs!
...
# Keeps Chat WebUI provider configs fully persistent across restarts!
...
# Preserves manually entered API keys on reboot while honoring Space Secrets
ENV_FILE="$HH/.env"
echo "[boot] 🔑 Synchronizing and merging environment credentials..."
touch "$ENV_FILE"
...
if [ -n "${TELEGRAM_BOT_TOKEN:-}" ]; then
  ...
  if curl -s -I --connect-timeout 5 "https://api.telegram.org" > /dev/null; then
    ...
    host api.telegram.org || getent hosts api.telegram.org || echo "DNS failed."
  fi
fi
    ...
    for fname in files:
        if fname.endswith((".html", ".py", ".js")):
            ...
            try:
                ...
                # Align any direct port calls to go directly through proxy
...
# ── 13. START ROUTER ───────────────────────────────────────────
echo "[boot] 🔀 Starting router on port ${PORT:-7860}..."
